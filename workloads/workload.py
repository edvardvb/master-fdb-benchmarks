...
class Workload(ABC):
    def __init__(self, db, runner, records, operations):
        ...
        print(
            f"🚄 Performing {operations} operations, using runner {runner} on {self.__repr__()}"
        )

    def benchmark(self, now):
        runtime, output = timeit.timeit(
            getattr(self, f"benchmark_{self.runner}"), number=1
        )
